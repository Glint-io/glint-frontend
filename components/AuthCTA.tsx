...
import { getStoredAuth } from "@/app/lib/auth";
...
export default function AuthCTA({ className = "" }: { className?: string }) {
    ...
    if (!isMounted) {
        return (
            <div
                aria-hidden
                className={"inline-flex h-10 items-center rounded-lg px-5 " + className + " invisible"}
            />
        );
    }
    ...
    return (
        <Link
            href="/auth/register"
            className={"inline-flex h-10 items-center rounded-lg border border-border bg-background px-5 font-mono text-sm font-medium text-foreground transition-colors hover:bg-background-subtle " + className}
        >
            Create free account
        </Link>
    );
