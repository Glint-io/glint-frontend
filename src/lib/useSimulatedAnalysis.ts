...
function createSimulatedAnalysis(): { result: AnalysisResult; jobLabel: string } {
  const random = createSeededRandom(0x6c696e74);
  const baseScore = random();
  const variance = 0.15;

  const score = Math.max(
    35,
    Math.min(98, Math.round(baseScore * 100 + (random() - 0.5) * variance * 100)),
  );

  const keywordScore = Math.max(
    30,
    Math.min(100, Math.round(score + (random() - 0.5) * 25)),
  );

  const rulesScore = Math.max(
    35,
    Math.min(100, Math.round(score + (random() - 0.5) * 20)),
  );
  ...
  const nextJobLabel = SAMPLE_JOBS[Math.floor(random() * SAMPLE_JOBS.length)];
...
const cachedAnalysis = createSimulatedAnalysis();
