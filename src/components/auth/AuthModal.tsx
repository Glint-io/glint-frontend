...
import { glintToast } from "../ui/toast";
...
export default function AuthModal() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 403) {
          ...
          glintToast.error({ message: "Incorrect email or password." });
        } else {
          const errorMessage = msg ?? `Login failed (${res.status}).`;
      ...
      setIsOpen(false);
      setShowOtp(false);
      glintToast.success({ message: "You have been signed in." });
      router.push("/user");
      router.refresh();
    } catch {
  ...
  async function handleResendCode() {
    ...
    if (result.ok) {
      ...
      setOtpDigits(Array(6).fill(""));
      glintToast.info({
        message: `A new code has been sent to ${registerEmail}.`,
      });
    } else {
      const errorMessage =
