...
import { authedGet, authedFetch, clearAuth, AUTH_KEY } from "@/app/lib/auth";
...
function ScoreBadge({ score }: { score: number }) {
    const color =
        score >= 80 ? "text-green-600 dark:text-green-400 bg-green-500/10 border-green-500/30"
            : score >= 60 ? "text-amber-600 dark:text-amber-400 bg-amber-500/10 border-amber-500/30"
...
function ResumeUpload({ onUploaded }: { onUploaded: () => void }) {
    ...
    const base = process.env.NEXT_PUBLIC_API_BASE_URL?.replace(/\/$/, "") ?? "https://localhost:7248";
    ...
    async function handleFile(file: File) {
        ...
        const res = await authedFetch(`${base}/user/resume`, { method: "POST", body: form });
        // Note: don't set Content-Type header here — browser sets it with boundary for multipart.
    ...
    return (
        <div>
            ...
            >
                <svg className="h-8 w-8 text-foreground-muted" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
                </svg>
                <span className="text-sm font-medium text-foreground">
                    {uploading ? "Uploading…" : "Upload a resume"}
                </span>
                <span className="text-xs text-foreground-muted">PDF · Max 5 MB · Drag & drop or click</span>
...
export default function UserPage() {
    ...
    useEffect(() => {
        try {
            ...
            const auth = JSON.parse(raw) as { accessToken?: string; payload?: { email?: string } };
            if (!auth.accessToken) { router.replace("/auth/login"); return; }
            const email = auth.payload && typeof auth.payload === "object"
                ? (auth.payload as Record<string, unknown>).email as string ?? null
                : null;
            setUserEmail(email);
        } catch {
    ...
    async function fetchAll(p = page) {
        ...
        try {
            const [s, h, r] = await Promise.all([
                ...
                authedGet<PaginatedHistory>(`/user/history?page=${p}&pageSize=${PAGE_SIZE}`),
    ...
    useEffect(() => { fetchAll(page); }, [page]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        <div className="mx-auto max-w-4xl space-y-8 px-4 py-10">
            ...
            <div className="flex items-start justify-between">
                ...
                <button
                    ...
                    className="rounded-md border border-border px-3 py-1.5 text-sm text-foreground-muted hover:border-foreground hover:text-foreground"
                >
                    Sign out
                </button>
            </div>
            ...
            <section>
                ...
                <div className="overflow-hidden rounded-xl border border-border bg-background">
                    {loading ? (
                        ...
                        <EmptyState icon="📄" message="No analyses yet. Upload a resume to get started." />
                    ) : (
                        <>
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-border text-left">
                                        <th className="px-5 py-3 font-medium text-foreground-muted">Date</th>
                                        <th className="px-5 py-3 font-medium text-foreground-muted">Job / Resume</th>
                                        <th className="px-5 py-3 text-right font-medium text-foreground-muted">Score</th>
                                    </tr>
                                </thead>
                                ...
                                <div className="flex items-center justify-between border-t border-border px-5 py-3 text-sm text-foreground-muted">
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className="rounded-md border border-border px-3 py-1 hover:bg-background-subtle disabled:opacity-40"
                                        >
                                            Previous
                                        </button>
                                        <button
                ...
                <div className="mt-4 overflow-hidden rounded-xl border border-border bg-background">
                    {loading ? (
                        ...
                        <ul className="divide-y divide-border">
                            {resumes.map((r) => (
                                <li key={r.resumeId} className="flex items-center gap-3 px-5 py-3">
                                    <svg className="h-4 w-4 shrink-0 text-foreground-muted" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                                    ...
                                    <button
                                        ...
                                        className="rounded-md p-1.5 text-foreground-muted hover:bg-red-500/10 hover:text-red-600 disabled:opacity-40"
